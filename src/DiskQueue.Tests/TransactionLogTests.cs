...
namespace DiskQueue.Tests
{
    ...
    public class TransactionLogTests : PersistentQueueTestsBase
    {
        ...
        public async Task Transaction_log_size_shrink_after_queue_disposed()
        {
            ...
            await using (var queue = await DiskQueue.Create(Path, Substitute.For<ILoggerFactory>(), paranoidFlushing: false).ConfigureAwait(false))
            {
        ...
        public async Task Count_of_items_will_remain_fixed_after_dequeueing_without_flushing()
        {
            ...
            {
                Assert.AreEqual(10, ((IDiskQueueStore)queue).EstimatedCountOfItemsInQueue);
            }
        }
        ...
        public async Task If_tx_log_grows_too_large_it_will_be_trimmed_while_queue_is_in_operation()
        {
            ...
            await using var queue = await DiskQueue.Create(Path, Substitute.For<ILoggerFactory>(), suggestedMaxTransactionLogSize: 32, paranoidFlushing: false)
        ...
        public async Task Truncated_transaction_is_ignored_with_default_settings()
        {
            ...
            await using (var queue = await DiskQueue
        ...
        public async Task Can_handle_truncated_start_transaction_separator()
        {
            ...
            await using (var queue = await DiskQueue.Create(Path, Substitute.For<ILoggerFactory>(), trimTransactionLogOnDispose: false)
                .ConfigureAwait(false))
            {
                using var session = queue.OpenSession();
                for (var j = 0; j < 20; j++)
        ...
        {
            await using (var queue = await DiskQueue.Create(Path, Substitute.For<ILoggerFactory>(), trimTransactionLogOnDispose: false).ConfigureAwait(false))
            {
                using var session = queue.OpenSession();
                for (var j = 0; j < 19; j++)
                {
            ...
            Can_restore_data_when_a_transaction_set_is_partially_overwritten_when_throwOnConflict_is_false()
        {
            ...
            await using (var queue = await DiskQueue.Create(Path, Substitute.For<ILoggerFactory>(), maxFileSize: Constants._32Megabytes, throwOnConflict: false)
                .ConfigureAwait(false))
            {
                using var session = queue.OpenSession();
        ...
        public async Task Will_remove_truncated_transaction()
        {
            ...
            var q = await DiskQueue.Create(Path, Substitute.For<ILoggerFactory>()).ConfigureAwait(false);
        ...
        public async Task Truncated_transaction_is_ignored_and_can_continue_to_add_items_to_queue()
        {
            ...
            await using (var queue = await DiskQueue.Create(Path, Substitute.For<ILoggerFactory>(), trimTransactionLogOnDispose: false, paranoidFlushing: false)
                .ConfigureAwait(false))
            {
                using var session = queue.OpenSession();
                for (var j = 0; j < 20; j++)
